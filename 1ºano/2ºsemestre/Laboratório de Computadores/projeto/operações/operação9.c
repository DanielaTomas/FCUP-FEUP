...
IMAGEM* add_border(IMAGEM* imagem1, FILE* f, char* argv[]) {
    int h = imagem1->altura, w = imagem1->largura;
    IMAGEM *imagem2 = (IMAGEM *) malloc(sizeof(IMAGEM)); //alocar memoria imagem rodada
    //ler imagem2
    imagem2->altura = h + atoi(argv[1]) + atoi(argv[1]);
    imagem2->largura = w + atoi(argv[1]) + atoi(argv[1]);
    imagem2->maxcor = imagem1->maxcor;
    imagem2->magicnumber[0] = imagem1->magicnumber[0];
    imagem2->magicnumber[1] = imagem1->magicnumber[1];
    imagem2->pixel = (PIXEL **) malloc(imagem2->altura * imagem2->largura * sizeof(PIXEL));
    for(int i = 0; i < imagem2->altura; i++) {
      imagem2->pixel[i] = (PIXEL *) malloc(imagem2->largura * sizeof(PIXEL));
    }
    for(int i = 0; i < imagem2->altura; i++) {
      for(int j = 0; j < imagem2->largura; j++) {
        if(i < atoi(argv[1]) || j < atoi(argv[1]) || i > imagem2->altura-1-atoi(argv[1]) || j > imagem2->largura-1-atoi(argv[1])) {
         imagem2->pixel[i][j].r = atoi(argv[2]);
         imagem2->pixel[i][j].g = atoi(argv[3]);
         imagem2->pixel[i][j].b = atoi(argv[4]);
        }
        else {
         imagem2->pixel[i][j].r = imagem1->pixel[i-atoi(argv[1])][j-atoi(argv[1])].r;
         imagem2->pixel[i][j].g = imagem1->pixel[i-atoi(argv[1])][j-atoi(argv[1])].g;
         imagem2->pixel[i][j].b = imagem1->pixel[i-atoi(argv[1])][j-atoi(argv[1])].b;
        }
      }
    }
    return imagem2;
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = add_border(imagem1,f,argv); //adicionar moldura -> operação 9
