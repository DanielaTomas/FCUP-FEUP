...
IMAGEM* rotate_diagonal(IMAGEM *imagem1) {
    ...
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        imagem2->pixel[h - i - 1][w - j - 1].r = imagem1->pixel[i][j].r;
        imagem2->pixel[h - i - 1][w - j - 1].g = imagem1->pixel[i][j].g;
        imagem2->pixel[h - i - 1][w - j - 1].b = imagem1->pixel[i][j].b;
      }
    }
    return imagem2;
}
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = rotate_diagonal(imagem1); //rodar diagonal -> operação 3
