...
IMAGEM* rotate_90_right(IMAGEM *imagem1) {
    int h = imagem1->largura, w = imagem1->altura;
    ...
    for(int i = 0; i < w; i++) {
      for(int j = 0; j < h; j++) {
        imagem2->pixel[j][w - i - 1].r = imagem1->pixel[i][j].r;
        imagem2->pixel[j][w - i - 1].g = imagem1->pixel[i][j].g;
        imagem2->pixel[j][w - i - 1].b = imagem1->pixel[i][j].b;
      }
    }
    return imagem2;
}
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = rotate_90_right(imagem1); //rodar 90 graus direita -> operação 7
