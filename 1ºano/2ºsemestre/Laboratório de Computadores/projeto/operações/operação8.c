...
IMAGEM* rotate_90_left(IMAGEM *imagem1) {
    ...
    for(int i = 0; i < w; i++) {
      for(int j = 0; j < h; j++) {
        imagem2->pixel[h - j - 1][i].r = imagem1->pixel[i][j].r;
        imagem2->pixel[h - j - 1][i].g = imagem1->pixel[i][j].g;
        imagem2->pixel[h - j - 1][i].b = imagem1->pixel[i][j].b;
      }
    }
    return imagem2;
}
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = rotate_90_left(imagem1); //rodar 90 graus esquerda -> operação 8
