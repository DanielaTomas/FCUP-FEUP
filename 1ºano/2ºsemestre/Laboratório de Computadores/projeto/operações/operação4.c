...
IMAGEM* adjust(IMAGEM* imagem1, FILE* f, char* argv[]) {
    ...
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        //atoi -> tranforma, neste caso, argv[] (char*) em inteiro (int)
        if(atoi(argv[1]) + imagem1->pixel[i][j].r > imagem1->maxcor) { //se superior a maxcor
          imagem2->pixel[i][j].r = imagem1->maxcor;
        }
        else if(atoi(argv[1]) + imagem1->pixel[i][j].r < 0) { //se inferior a zero
          imagem2->pixel[i][j].r = 0;
        }
        else imagem2->pixel[i][j].r = atoi(argv[1]) + imagem1->pixel[i][j].r;
        if(atoi(argv[2]) + imagem1->pixel[i][j].g > imagem1->maxcor) {
          imagem2->pixel[i][j].g = imagem1->maxcor;
        }
        else if(atoi(argv[2]) + imagem1->pixel[i][j].g < 0) { //se inferior a zero
          imagem2->pixel[i][j].g = 0;
        }
        else imagem2->pixel[i][j].g = atoi(argv[2]) + imagem1->pixel[i][j].g;
        if(atoi(argv[3]) + imagem1->pixel[i][j].b > imagem1->maxcor) { //se superior a maxcor
          imagem2->pixel[i][j].b = imagem1->maxcor;
        }
        else if(atoi(argv[3]) + imagem1->pixel[i][j].b < 0) { //se inferior a zero
          imagem2->pixel[i][j].b = 0;
        }
        else imagem2->pixel[i][j].b = atoi(argv[3]) + imagem1->pixel[i][j].b;
      }
    }
    return imagem2;
}
...
int main(int argc, char* argv[]) { //função principal
    ...
    if(argc > 5) {
     f = fopen(argv[4],"r"); //abrir ficheiro f
     if(f == NULL) {
      fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[4]); //erro ao abrir
      exit(1);
     }
     output = fopen(argv[5],"w"); //abrir ficheiro output
     if(output == NULL) {
      fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[5]); //erro ao abrir
      exit(1);
     }
    }
    else if(argc > 4) {
    ...
    imagem2 = adjust(imagem1,f,argv); //ajustar imagem -> operação 4
