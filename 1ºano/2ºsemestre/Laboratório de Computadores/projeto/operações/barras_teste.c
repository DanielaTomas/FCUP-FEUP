...
IMAGEM* riscas(IMAGEM* imagem1, FILE* f, char* argv[]) {
    ...
    //ler pixeis imagem e colocar riscas horizontais na imagem
    int k = atoi(argv[1]);
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        if(((i+k) % (k*2)) < k) { //trocar i pelo j para as riscas ficarem verticais
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = riscas(imagem1,f,argv); //adicionar moldura -> operação 9
