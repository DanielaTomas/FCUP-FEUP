...
IMAGEM* black_white(IMAGEM* imagem1, FILE* f, char* argv[]) {
    ...
    //ajustar cores imagem <--> ler pixeis imagem2
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        int g = (0.2126*imagem1->pixel[i][j].r) + (0.7152*imagem1->pixel[i][j].g) + (0.0722*imagem1->pixel[i][j].b ) + 0.5;
        if(g > atoi(argv[1])) {
...
int main(int argc, char* argv[]) { //função principal
    ...
    if(argc > 3) {
     f = fopen(argv[2],"r"); //abrir ficheiro f
     if(f == NULL) {
      fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[2]); //erro ao abrir
      exit(1);
     }
     output = fopen(argv[3],"w"); //abrir ficheiro output
     if(output == NULL) {
      fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[3]); //erro ao abrir
      exit(1);
     }
    }
    else if(argc > 2) {
    ...
    imagem2 = black_white(imagem1,f,argv); //preto e branco -> operação 6
