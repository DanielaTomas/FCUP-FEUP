...
IMAGEM* grey_scale(IMAGEM* imagem1, FILE* f) {
    ...
    //grey scale <--> ler pixeis imagem2
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        if((0.2126*imagem1->pixel[i][j].r + 0.7152*imagem1->pixel[i][j].g + 0.0722*imagem1->pixel[i][j].b) + 0.5 >= imagem1->maxcor) { //se superior a maxcor
         imagem2->pixel[i][j].r = imagem1->maxcor;
         imagem2->pixel[i][j].g = imagem1->maxcor;
         imagem2->pixel[i][j].b = imagem1->maxcor;
        }
        else {
         imagem2->pixel[i][j].r = 0.2126*imagem1->pixel[i][j].r + 0.7152*imagem1->pixel[i][j].g + 0.0722*imagem1->pixel[i][j].b + 0.5;
         imagem2->pixel[i][j].g = 0.2126*imagem1->pixel[i][j].r + 0.7152*imagem1->pixel[i][j].g + 0.0722*imagem1->pixel[i][j].b + 0.5;
         imagem2->pixel[i][j].b = 0.2126*imagem1->pixel[i][j].r + 0.7152*imagem1->pixel[i][j].g + 0.0722*imagem1->pixel[i][j].b + 0.5;
        }
      }
    }
    return imagem2;
...
int main(int argc, char* argv[]) {
    ...
    imagem2 = grey_scale(imagem1,f); //grey scale -> operação 5
