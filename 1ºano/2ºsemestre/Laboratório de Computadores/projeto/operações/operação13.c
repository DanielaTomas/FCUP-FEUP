...
IMAGEM* transparencia(IMAGEM* imagem1, IMAGEM* imagem2, char* argv[]) {
    ...
    for(int i = 0; i < imagem3->altura; i++) {
      ...
      for(int j = 0; j < imagem3->largura; j++) {
        if((i >= atoi(argv[3]) && j >= atoi(argv[2])) && (y < imagem1->altura) && (x < imagem1->largura)) {
         aux = 1;
         if(alpha * imagem1->pixel[y][x].r + (1-alpha) * imagem2->pixel[i][j].r + 0.5 >= imagem3->maxcor)
           imagem3->pixel[i][j].r = imagem3->maxcor;
         else {
          imagem3->pixel[i][j].r = alpha * imagem1->pixel[y][x].r + (1-alpha) * imagem2->pixel[i][j].r + 0.5;
         }
         if(alpha * imagem1->pixel[y][x].g + (1-alpha) * imagem2->pixel[i][j].g + 0.5 >= imagem3->maxcor)
           imagem3->pixel[i][j].g = imagem3->maxcor;
         else {
          imagem3->pixel[i][j].g = alpha * imagem1->pixel[y][x].g + (1-alpha) * imagem2->pixel[i][j].g + 0.5;
         }
         if(alpha * imagem1->pixel[y][x].b + (1-alpha) * imagem2->pixel[i][j].b + 0.5 >= imagem3->maxcor)
           imagem3->pixel[i][j].b = imagem3->maxcor;
         else {
          imagem3->pixel[i][j].b = alpha * imagem1->pixel[y][x].b + (1-alpha) * imagem2->pixel[i][j].b + 0.5;
         }
         x++;
       }
       else {
...
int main(int argc, char* argv[]) { //função principal
    ...
    IMAGEM *imagem3 = (IMAGEM *) malloc(sizeof(IMAGEM)); //alocar memoria imagem rodada
    if(argc == 7){
      f1 = fopen(argv[4],"r"); //abrir ficheiro f1
      if(f1 == NULL) {
      ...
      output = fopen(argv[6],"w"); //abrir output
      if(output == NULL) {
        fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[3]);
        exit(1);
      }
      ...
      f2 = fopen(argv[5],"r"); //abrir ficheiro f2
      if(f2 == NULL) {
        fprintf(stderr,"%s ficheiro falhou a abrir ou não existe!!\n",argv[2]);
        exit(1);
      }
    ...
    imagem3 = transparencia(imagem1,imagem2,argv); //transparencia imagem -> operação 13
